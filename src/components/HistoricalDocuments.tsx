...
const HistoricalDocuments: React.FC<HistoricalDocumentsProps> = ({ companyId, readOnly = false }) => {
  const [items, setItems] = useState<HistoricalDocument[]>(() => {
    const savedData = localStorage.getItem(`historical_${companyId}`);
    return savedData ? JSON.parse(savedData) : [];
  });

  useEffect(() => {
    localStorage.setItem(`historical_${companyId}`, JSON.stringify(items));
  }, [items, companyId]);
  ...
  const [selectedItem, setSelectedItem] = useState<HistoricalDocument | null>(null);
  const [editingItem, setEditingItem] = useState<HistoricalDocument | null>(null);
  ...
  const handleEdit = (item: HistoricalDocument) => {
    setEditingItem({ ...item });
    setShowEditModal(true);
  };

  const handleDelete = (item: HistoricalDocument) => {
